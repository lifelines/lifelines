...
"--------------------------------------------------------------------------\n")
    ...
    getintmsg(who,"over set (0=all, 1=descendants, 2=ancestors, 3=indiset) ")
    ...
    while (1) {
        ...
        if (not(strcmp(restriction_equation,""))) { break() }
    ...
    sepline
    "all" col(6)
    if (gt(deltabin,0)) { dd(minbin,allycount) col(18) dd(maxbin,allycount) }
    else { xstring }
    col(30) d(allycount)
...
func get_val(person,spec_string) {
    ...
    if (eq(event,1)) {
        ...
        if (not(strcmp(spec_string,concat(initial,"MONTH")))) {
        ...
        if (not(strcmp(spec_string,concat(initial,"DAY")))) {
            if (day) { set(ret_type,2) return(day) }
            return(0)
        }
        if (not(strcmp(spec_string,concat(initial,"AGE")))) {
            set(byear,0)
        ...
        if (not(strcmp(spec_string,concat(initial,"FAGE")))) {
            set(byear,0)
        ...
        if (not(strcmp(spec_string,concat(initial,"MAGE")))) {
            set(byear,0)
        ...
        if (not(strcmp(spec_string,concat(initial,"PLACE")))) {
    ...
    if (not(strcmp(spec_string,"FAMILIES"))) {
        set(ret_type,2)
        return(nfamilies(person))
    }
    if (not(strcmp(spec_string,"SIBLINGS"))) {
    ...
    if (not(strcmp(spec_string,"SIBLINGORDER"))) {
        if (fam,parents(person)) {
            set(ret_type,2)
            set(cnum,1)
            set(child,person)
...
/* The next four lines will work even if you comment them out.
   Sex is a powerful force, I guess. */
    ...
    if (not(strcmp(spec_string,"SOUNDEX"))) {
        set(ret_type,1)
        return(save(soundex(person)))
    }
    fornodes(inode(person),node) {
        if (not(strcmp(tag(node),spec_string))) {
            set(ret_type,1)
    ...
    forlist(rlist,restriction,rnum) {
        ...
        if (eq(ret_type,rtype)) {
            if (eq(rtype,2)) { /* numeric */
                ...
                elsif (not(strcmp(inequality,"contains"))) {
                    if (not(index(ret_val,cvalue,1))) { return(0) }
                }
                elsif (not(strcmp(inequality,"isin"))) {
